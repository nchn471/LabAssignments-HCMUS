...
class Point(object):
    ...
    def __eq__(self, point): #equal
        return point and self.x == point.x and self.y == point.y
    
    def __ne__(self,point): #not_equal
...
class Graph:
    def __init__(self, polygons):
        ...
        for polygon in polygons:
            ...
            if len(polygon) == 2:
                polygon.pop()
            # first point = last point (start or goal)
            if polygon[0] == polygon[-1]:
                self.add_point(polygon[0])
            else:
                for i, point in enumerate(polygon):
                    neigbor_point = polygon[(i+1) % len(polygon)]
                    edge = Edge(point, neigbor_point)
                    if len(polygon) > 2:
                        point.polygon_id = pid
                        neigbor_point.polygon_id = pid
...
# using for astar, greedy-bfs, ucs 
...
def main():
    ...
    with open('Input.txt','r') as f:
        ...
        # Uncomment the line for the search algorithm (A*,GBFS, BFS, DFS, UCS) you want   
    
        # a = search(graph, start, goal, a_star) # astar
        # a = search(graph, start, goal, greedy)  # gbfs
        # a = DFS_search(graph,start,goal) # DFS
        # a = BFS_search(graph,start,goal) # BFS
        a = UCS_search(graph,start,goal) # UCS
        # or 
