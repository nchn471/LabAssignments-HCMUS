...
class Graph():
    ...
    def primMST(self, d_temp, t):
        ...
        for c in range(self.V):
            # Pick the minimum distance vertex from the set of vertex not yet processed.
            # u is always equal to src in first iteration
            u = self.minKey(key, mstSet)
            # Put the minimum distance vertex in the shortest path trê
            mstSet[u] = True
...
def checkPath(tree, toExpand, V):
    ...
    if (tnode.data.c_id == 1):
        ...
        # do up in the trê using the parent pointer and add all 
...
def startTSP(graph, tree, V):
    ...
    while (goalState == 0):
        ...
        path = checkPath(tree, toExpand, V) # check path of tselected node if it is complete or not
